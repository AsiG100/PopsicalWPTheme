<?php
/* Template Name: Concerts Landing Page - Checkout Button */
...
<div class="concerts-page-container">
    ...
    <header class="concert-header">
        ...
        <button id="trigger-eventbrite-widget-<?php echo esc_attr($current_event_id_for_widget); ?>" class="cta-button trigger-eventbrite-widget" data-event-id="<?php echo esc_attr($current_event_id_for_widget); ?>">Reserve Your Tickets</button>
        <?php else: ?>
                ...
                <div class="concerts-list" id="concerts-list">
                    <?php foreach ($events_to_list_in_main_section as $event_item): ?>
                        ...
                        <div class="concert-item">
                            ...
                            ?>
                            <?php if ($is_available): ?>
                                <a href="<?php echo esc_url($event_page_link); ?>" id="trigger-eventbrite-widget-<?php echo esc_attr($item_id); ?>" class="details-button trigger-eventbrite-widget" data-event-id="<?php echo esc_attr($item_id); ?>">Reserve Your Spot</a>
            ...
            <div class="other-concerts-grid">
                <?php foreach ($other_concerts_to_list as $other_event): ?>
                    ...
                    <div class="other-concert-item " data-event-id="<?php echo esc_attr($other_event_id); ?>">
                        ...
                        ?>
                        <?php if ($is_available): ?>
                            <a href="<?php echo esc_url($event_page_link); ?>" id="trigger-eventbrite-widget-<?php echo esc_attr($other_event_id); ?>" class="details-button trigger-eventbrite-widget" data-event-id="<?php echo esc_attr($other_event_id); ?>">Reserve Your Spot</a>
                        <?php elseif ($is_sold_out): ?>
                            <span class="details-button disabled sold-out">Sold Out</span>
                        <?php else: ?>
                            <span class="details-button disabled unavailable">Unavailable</span>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Load Eventbrite modal script if not already loaded
    function loadEventbriteModalScript(callback) {
        if (window.EBWidgets) {
            callback();
            return;
        }
    ...
    clickableItems.forEach(item => {
        item.addEventListener('click', function(e) {
            ...
            loadEventbriteModalScript(function() {
                window.EBWidgets.createWidget({
                    widgetType: 'checkout',
                    eventId: eventId,
                    modal: true,
                    modalTriggerElementId: 'trigger-eventbrite-widget-' + eventId
                });
            });
        });
    });
